...
export class TooltipComponent {
  ...
  public showOnboardingIfNeeded(mapName: String) {
    ...
    const shouldShowOnboarding = localStorage.getItem('firstVisit') == null && window.innerWidth > 500;
  ...
  continueOnboarding() {
    ...
    if (!this.hasShownTabsTooltip) {
      ...
      let menuGroups = document.getElementById('menu-groups-tabs') as HTMLDivElement;
      if (!menuGroups) {
        this.continueOnboarding();
        return;
      }
    ...
    if (!this.hasShownMenusTooltip) {
      ...
      let menus = document.getElementById('menu-button') as HTMLDivElement;
      if (!menus) {
        this.continueOnboarding();
        return;
      }
    ...
    if (!this.hasShownLocationTooltip) {
      ...
      if (location) {
        ...
        this.addTooltipNextTo(
          location,
          'In the map, there are geolocated points, known as <b>Locations</b>.<br>Through their colors you will be able to see which <i>tags</i> they relate to.<br> Clicking a location opens up a <b>popup</b> with further details.<br> Thanks for your visit!</b>'
        );
